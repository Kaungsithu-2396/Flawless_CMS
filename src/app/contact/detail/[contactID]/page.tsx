...
export default async function page({
    ...
    async function getDetailContact() {
        try {
            ...
            return contactDetailResp.data.data;
        } catch (error) {
            console.log(error);
        }
    }
    const contactData = await getDetailContact();
                ...
                <div className="hidden md:block">
                    <div className="flex   justify-center items-stretch w-full m-auto  gap-8 md:gap-10">
                        ...
                        <div className="font-bold">
                            ...
                            <p className=" my-9">{contactData.phoneNumber}</p>
                            <p className=" my-9">{contactData.message}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="md:hidden block my-5">
                    <div className="flex flex-col justify-center items-start gap-4">
                        <span className="flex justify-start items-center">
                            ...
                            <h1 className="font-bold">{contactData.name}</h1>
                        </span>
                        <span className="flex  items-center">
                            <h1 className="mx-3 mr-5">Email </h1>{" "}
                            <h1 className="font-bold"> {contactData.email}</h1>
                        </span>{" "}
                        ...
                        <span className="flex justify-around items-center">
                            ...
                            <h1 className="font-bold">{contactData.message}</h1>
                        </span>
                    </div>
                </div>
            </section>
